...
| 符号 | 位置 | 语义 |
|------|------|------|
| `jeffies_get()` | `core/kernel/time/time.c` | Soft tick：`(arch_timer_read() - per_cpu(boot_base_time, BSP)) / (clock_hz/INT_PER_SECOND)`；**不**由 IRQ 递增 |
| `tick_cnt` (percpu) | 同上 | 本核 soft tick：heartbeat 时用本核 `boot_base_time` 刷新；与全局 jeffies 同源公式 |
| `SYS_TIME_MS_PER_INT` = 10 ms | `core/include/rendezvos/time.h` | soft / heartbeat 目标粒度 |
| `INT_PER_SECOND` = 100 | 同上 | |
| `udelay` / `mdelay` | `time.c` | **忙等**，非 sleep；`loop_per_jeffies` 由 arch 校准 |
| `rendezvos_do_time_irq` | `time.c` | 定时器 IRQ → event RB-tree；**不**维护 soft jeffies |
...
| 能力 | 实现 |
|------|------|
| MONOTONIC | `rendezvos_time_now()` → us（现行；不再用 jeffies×10ms） |
| REALTIME | x86: 启动 RTC 快照 + monotonic；aarch64: PL031 / 回退 |
| nanosleep | timer event + `recv_msg`（§10）；非 jeffies 轮询 |
| times | tms 存根；返回值 = `jeffies_get()`；`sysconf(_SC_CLK_TCK)` 与 `INT_PER_SECOND` 对齐（100） |
...
| **T1** | 档 A：`gettimeofday` + `times` + sleep | 否 | #16/#20；#17 部分 |
...
4. SMP: soft jeffies 为 arch 只读推导，无跨 CPU CAS；ktime read must be coherent
```
