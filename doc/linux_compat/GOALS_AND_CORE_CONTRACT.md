...
**不追求：** 在 core 内复刻 Linux（无独立 VMA 子系统、无完整 swap、无 `cancel_ipc` 除非将来真需要）。
...
| **wait4** | 可阻塞在 port；依赖对端 exit 路径发消息，**不依赖** `cancel_ipc`（当前 core 未实现该 API）。 |
| **无 cancel_ipc** | 线程/端口上的「硬取消」由 exit + matcher 丢弃 request 承担；若将来 libc 需要可中断阻塞，再单独设计（可能新 core 原语，见 §5）。 |
