...
static vaddr signal_build_frame(vaddr base_sp, int sig, void *handler)
{
        (void)sig;
        (void)handler;
...
bool linux_signal_wait4_should_return_eintr(Thread_Base *thread)
{
        ...
        if (sig == SIGCHLD) {
                disp = &ps->dispositions[SIGCHLD - 1];
...
bool linux_deliver_pending_signals(struct trap_frame *tf)
{
        ...
        user_sp = signal_build_frame(user_sp, sig, disp->sa_handler);
