               ...
               u64 new_address)
{
        ...
        void* new_pages =
                get_free_page(expand_size / PAGE_SIZE,
                              (vaddr)expand_addr,
                              percpu(nexus_root),
                              tcb->vs,
                              PAGE_ENTRY_USER | PAGE_ENTRY_VALID
        ...
        if (!(flags & LINUX_MREMAP_MAYMOVE)) {
                pr_debug(
                        "[mremap] can't expand in place and MAYMOVE not set\n");
                return -LINUX_ENOMEM;
        }
        ...
        void* new_mapping =
                get_free_page(new_size_aligned / PAGE_SIZE,
